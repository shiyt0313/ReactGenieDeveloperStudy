...
##### 2.3 Initial Set Up
1. Please ensure that you have the latest version of `reactgenie-lib` in `package.json`. 
...
2. Update your API Keys for Azure and Open AI in the `.env` file with your credentials. 
See `.env.example` for reference. You can use the command below to create an .env file with credentials.
...
npx expo start --web
```
If there is something wrong with the dependencies, you can delete the `node_modules` folder and run the command again.
