...
Note that, a state class in ReactGenie has to extend `DataClass` and be annotated with `@GenieClass("{description of the class}")` to help ReactGenie better understand the role of the class.
In the counter, we create three class properties type, name, and count.

Another thing to keep in mind is that ReactGenie will handle the creation, management, update, and storage of all state classes (`DataClass`es), so we only support primitive types (e.g., `string`, `int`, `float`, `boolean`), arrays of primitive types, compound types (e.g., `DateTime` and `TimeDelta`), and arrays of compound types.
...
In ReactGenie, this is done through annotating the properties with `@GenieProperty("{description of the property}")`.
...
Another thing is required for a `DataClass` is that it has to have a key for ReactGenie to manage the data.
The key is like key column in a database, it has to be unique for each instance of the class.
In ReactGenie, this is done through annotating the key with `@GenieKey`.
...
To recap, a state class (`DataClass`) in ReactGenie have to have the following three things:
* `@GenieClass("{description of the class}")` and extends `DataClass`
* `@GenieProperty("{description of the property}")`: properties that you want to expose to ReactGenie
* `@GenieKey`: a unique key for each instance of the class

### 2.4 State class management

Similar to TypeScript, ReactGenie `DataClass`es need a constructor.
...
Note that for `DataClass`, all function parameters have to be destructured, this allows you to have parameters that are optional.

```typescript
// without Destructuring
function example(a: number, b: number){}
// with Destructuring
function example({a, b}: {a: number, b: number}){}
// with Destructuring and default value
function example({a, b = 0}: {a: number, b?: number}){}
```
...
`GetObject()` allows you to access a object through its `GenieKey`.
`All()` allows you to get an array of all instances of a specific class.
`DeleteObject()` allows you to delete a object through its `GenieKey`.
...
```
*TODO: update this*
Note that similar to `GenieProperty`, `GenieFunction` is used to expose functions to ReactGenie that users can increment and decrement the counter via multimodal interactions.
...
`useGenieSelector()`: wrapper function used to access ReactGenie data

`genieDispatch()`: wrapper function used when modifying the ReactGenie data or calling a function
...
```
You can also rename `store.ts` to whatever you want, but don't forget to import it in `App.tsx` and use it with the Provider
