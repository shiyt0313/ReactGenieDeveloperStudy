...
const CounterViewImpl = (props: { name: string}) => {
    ...
    return (
        <div>
            <label>[{counterType}]</label> <br/> <label>{props.name}: </label> <br/> <label> {counterValue}</label>
...
const CounterViewImpl = (props: { name: string}) => {/*UI code*/}
...
const CounterListViewImpl = (props: { elements: { name: string }[] }) => {/*UI code*/}
...
<Provider store={reactGenieStore}>//load your own store
    <ModalityProvider
        ...
        >
        <CounterListView elements={Counter.All()}> // you UI code
    </ModalityProvider>
</Provider>
```
