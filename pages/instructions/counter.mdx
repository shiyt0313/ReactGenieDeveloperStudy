...
<font color="red">The first part involves the specific implementation of the UI, and the second part deals with binding the UI with data types.</font>
...
Below are files that demonstrate the implementation of UI and get data using `useGenieSelector`.
...
```typescript filename="EditCounterView.tsx" {6,7,8,15,18,21,25}
...
```
Please avoid testing one function with two similar utterance examples.
